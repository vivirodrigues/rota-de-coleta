...
def update_weight(G, vehicle_mass):
    ...
    for u, v, k, data in G.edges(keys=True, data=True):
        weight_value = work(vehicle_mass, data['surface'], data['grade'], data['length'])
...
def config_graph(G, geotiff_name, stop_points, ad_weights, file_name_osm, G_file, net_file):

    G, nodes_coordinates, nodes_weights = Scenario.add_collect_points(G, stop_points, ad_weights, file_name_osm)

    Simulation.netconvert_geotiff(file_name_osm, MAPS_DIRECTORY + 'out.tif', net_file)

    G = Scenario.simulation_edit_graph(G, file_name_osm, net_file)
