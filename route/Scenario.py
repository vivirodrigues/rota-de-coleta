    ...
    for i in collect_points:
        ...
        if len(nodes_adjacent) > 1:
            ...
            if len(edge_split) > 1:
                ...
                G.add_edge(id_nearest_node, int(first_node), length=len_first_edge, highway=highway, oneway='false')
                G.add_edge(int(first_node), id_nearest_node, length=len_first_edge, highway=highway, oneway='false')

                # create the edge of the second adjacent node
                # to the closest node inside the way
                G.add_edge(id_nearest_node, int(second_node), length=len_second_edge, highway=highway, oneway='false')
                G.add_edge(int(second_node), id_nearest_node, length=len_second_edge, highway=highway, oneway='false')
                ...
                osm_tag = create_way(osm_tag, str(id_1adjacent_street1), str(int(first_node)), str(int(id_nearest_node)))
                osm_tag = create_way(osm_tag, str(id_1adjacent_street2), str(int(id_nearest_node)), str(int(first_node)))

                # edges between nearest node and second id node
                osm_tag = create_way(osm_tag, str(id_2adjacent_street1), str(id_nearest_node), str(int(second_node)))
                osm_tag = create_way(osm_tag, str(id_2adjacent_street2), str(int(second_node)), str(id_nearest_node))
                ...
                G = _add_node(G, i, id_node_collect)
                # edge between node collect to nearest node dividing the adjacent street
                len_edge = calculate_distance(i, (G.nodes[id_node]['y'], G.nodes[id_node]['x']))
                G.add_edge(id_node_collect, id_node, length=len_edge, highway='service', oneway='false')
                G.add_edge(id_node, id_node_collect, length=len_edge, highway='service', oneway='false')
                if str(id_nearest_node) == str(node1_added):
                    osm_tag = create_node(osm_tag, str(0), str(0), str(0))

                osm_tag = create_node(osm_tag, str(id_node_collect), str(i[0]), str(i[1]))
                # edges between nearest node and first id node
                osm_tag = create_way(osm_tag, str(id_0adjacent_street1), str(int(id_node)), str(int(id_node_collect)))
                osm_tag = create_way(osm_tag, str(id_0adjacent_street2), str(int(id_node_collect)), str(int(id_node)))

                lat = float(i[0]) + 0.00002
                lon = float(i[1]) + 0.00002
                osm_tag = create_node(osm_tag, str(int(id_node_collect + 50)), str(lat), str(lon))
                # edges between nearest node and first id node
                osm_tag = create_way(osm_tag, str(int(id_0adjacent_street1 + 50)), str(int(id_node)), str(int(id_node_collect + 50)))
                osm_tag = create_way(osm_tag, str(int(id_0adjacent_street2 + 50)), str(int(id_node_collect + 50)), str(int(id_node)))
                osm_tag = create_way(osm_tag, str(int(id_0adjacent_street1 + 300)), str(int(id_node_collect)), str(int(id_node_collect + 50)))
                osm_tag = create_way(osm_tag, str(int(id_0adjacent_street2 + 300)), str(int(id_node_collect + 50)), str(int(id_node_collect)))
                ##

                if id_node in list(nodes_collect_coordinates.keys()):
                    lat = float(i[0]) + 0.02
                    lon = float(i[1]) + 0.02
                    id_node, dist = ox.get_nearest_node(G, (lat, lon), return_dist=True)

                if i in ad_weights:
                    weight = ad_weights.get(i)[0]

                #if id_node not in list(nodes_collect_coordinates.keys()):
...
def simulation_edit_graph(G, file_name_osm, file_name_net):
    dict_edges_net = edges_net(file_name_net)
    dict_edges_type = edges_type(file_name_net)
        ...
        if nodes_adjacent_xml.get(i) is not None and len(nodes_adjacent_xml.get(i)) > 0:
            for j in nodes_adjacent_xml.get(i):
                ...
                if G.has_edge(*e1) is False and BIDIRECTIONAL is True:
                    ...
                    edge_type = dict_edges_type.get(e)
                    if edge_type != ['subway'] and edge_type != ['rail']: # and edge_type != ['steps']
